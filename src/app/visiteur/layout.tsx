    ...
    <CartProvider>
      <GuestGuard>
        <div className="flex flex-col min-h-screen bg-app-surface font-sans text-app-primary selection:bg-app-accent/20">
          ...
          {/* NAVBAR ÉPURÉE (Glassmorphism & Sticky) */}
          <header className="sticky top-0 z-50 h-20 bg-app-surface/80 backdrop-blur-md border-b border-app-primary/5 flex items-center justify-between px-4 md:px-8 transition-all">
            ...
            <div className="flex items-center gap-2 md:gap-4 ml-4 md:ml-8">
              ...
              {/* Bouton Panier avec pastille */}
              <button className="relative h-10 w-10 flex items-center justify-center rounded-full text-app-secondary hover:text-app-primary hover:bg-app-primary/5 transition-all active:scale-95">
                <ShoppingCart size={22} />
                {/* Petite pastille pour indiquer un article (Optionnel, tu pourras le lier à ton CartContext plus tard) */}
                <span className="absolute top-2 right-2 h-2 w-2 rounded-full bg-app-accent border-2 border-app-surface"></span>
              </button>
              
              {/* Bouton Connexion style "Pillule" */}
              <Link 
                href="/login" 
          ...
          {/* CONTENU PRINCIPAL */}
          <main className="flex-1 w-full max-w-7xl mx-auto p-6 md:p-8 pt-8 md:pt-12">
            {children}
          </main>
